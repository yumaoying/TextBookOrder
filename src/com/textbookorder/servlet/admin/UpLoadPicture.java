...
public class UpLoadPicture extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			/*
			 * if (textBook.getId() == 0) { errors.put("id", "未获取图书编号，无法上传！"); }
			 */
			if (errors.size() > 0) {
				request.setAttribute("errors", errors);
				// request.setAttribute("textBook", textBook);
				request.getRequestDispatcher(
						"/admin/AdminTextBookServlet?method=load&id="
								+ textBook.getId()).forward(request, response);
				return;
			}
			// // 使用目录和文件名创建目标文件
			...
			bookservice.uploadImg(textBook.getId(), textBook.getBookpicture());
